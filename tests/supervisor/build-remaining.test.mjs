...
test("runBuild: a spawn failure (bad cwd) resolves ok:false with a 'failed to spawn' reason", async () => {
  const result = await runBuild("/nonexistent/not-a-real-directory-at-all", "bos/testfixture-spawn-fail");
...
test("runBuild: a SYNCHRONOUS spawn throw (invalid cwd type) is caught, not left to crash the caller", async () => {
  ...
  const result = await runBuild(12345, "bos/testfixture-spawn-sync-throw");
  assert.equal(result.ok, false);
  assert.equal(result.code, null);
  assert.match(result.reason, /failed to spawn build/);
});
...
test("runBuild: a huge failing build's output tail is bounded, not unbounded", async () => {
  ...
  const result = await runBuild(bigOutputDir, "bos/testfixture-big-output");
  assert.equal(result.ok, false);
...
test("buildAndStart: a spec-store commit failure is recorded as a warning but does not abort the build", async () => {
  const branch = "bos/testfixture-spec-warn";
...
test("buildAndStart: a genuine (non-'nothing to commit') commit failure aborts and names the real reason", async () => {
  const branch = "bos/testfixture-commit-hook-fails";
  git(env.repo, ["branch", branch]);
  const wt = await addWorktreeForBranch(branch);
...
test("buildAndStart: a live-checkout safety-gate violation aborts before any build step, and restores REPO", async () => {
  const branch = "bos/testfixture-safety-gate";
  git(env.repo, ["branch", branch]);
  const wt = await addWorktreeForBranch(branch);
  ...
  git(env.repo, ["checkout", "-q", "-b", "bos/testfixture-unrelated-drift"]);
  try {
    const v = fakeVersion({ worktree: wt, branch, role: "preview" });
    const result = await buildAndStart(v);
    assert.equal(result, "failed");
    ...
    git(env.repo, ["branch", "-D", "bos/testfixture-unrelated-drift"]);
  }
});
...
test("buildAndStart: a successful build reaches 'ready' via a real health-gated server", async () => {
  ...
  try {
    const branch = "bos/testfixture-build-ready";
    git(env.repo, ["branch", branch]);
    const wt = await addWorktreeForBranch(branch);
...
test("buildAndStart: a build that succeeds but never becomes healthy fails with a health-check-specific reason", async () => {
  ...
  try {
    const branch = "bos/testfixture-build-unhealthy";
    git(env.repo, ["branch", branch]);
    const wt = await addWorktreeForBranch(branch);
    const pkgPath = join(wt, "package.json");
    writeFileSync(pkgPath, JSON.stringify({ name: "fake-bos", scripts: { build: 'node -e "process.exit(0)"' } }));
...
test("buildAndStart: a preview that DIES after starting reports the exit, not a health timeout", async () => {
  // The live failure: BOS's own preview exited 0.7s after "Ready in 272ms",
  // because a marketplace item's service entrypoint ran `process.exit(0)` when
  // it found no parentPort — and BOS's load check imported it on the main
  // thread. What the user was shown was
  //     "health check failed: no healthy /api/health on :3001 within 120000ms"
  // which describes a 120-second wait that never happened and points at the
  // wrong subsystem entirely.
  const { restore } = installFakeNpx();
  process.env.FAKE_SERVER_EXITS_AFTER_MS = "300";
  try {
    const branch = "bos/testfixture-build-dies-early";
    git(env.repo, ["branch", branch]);
    const wt = await addWorktreeForBranch(branch);
    writeFileSync(join(wt, "package.json"), JSON.stringify({ name: "fake-bos", scripts: { build: 'node -e "process.exit(0)"' } }));
    git(wt, ["add", "-A"]);
    git(wt, ["commit", "-q", "-m", "fast build script"]);

    const v = fakeVersion({ worktree: wt, branch, role: "preview", port: 45904 });
    const result = await buildAndStart(v);
    assert.equal(result, "failed");
    assert.match(v.buildError, /exited before becoming healthy/);
    assert.doesNotMatch(v.buildError, /within \d+ms/, "must not claim a timeout that never elapsed");
  } finally {
    delete process.env.FAKE_SERVER_EXITS_AFTER_MS;
    restore();
  }
});
