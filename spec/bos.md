...
- **Installing apps**: an installed app is a self-contained folder (entry `index.html`) stored in the **apps content repo** (GitFS — `spec/self-modification/gitfs.md`, root `BOS_APPS_DIR`, default `<cwd>/apps`), not in the VFS, and served same-origin at `/apps/<id>`, rendered in a window as an iframe. Apps are **discovered by listing that directory** (no central registry), and every install/uninstall/restore/purge is a git commit, so apps are versioned content separate from BOS source. The assistant installs apps with `installApp` after the Developer builds them (the "Build App" skill). On install the app gets an appropriate icon and the desktop refreshes so the icon appears.
