...
export function DocsActions() {
  useCopilotAction({
    ...
    parameters: [],
    handler: () =>
      runToolHandler("docs_list", async ({ signal }) => {
        const out = await fetchToolJson("docs_list", "/api/docs", { signal });
        if (!out.ok) return out.error;
        const res = out.data as { tree?: Record<string, DocNode[]> };
  ...
  useCopilotAction({
    ...
    handler: ({ ref }) =>
      runToolHandler("docs_read", async ({ signal }) => {
        ...
        if (!out.ok) return out.error;
        const res = out.data as { doc?: { content: string }; error?: string };
