...
export function RunCommandActions() {
  useCopilotAction({
    name: "run_command",
    description:
      "Run a command in a sandboxed environment (Settings → Command Execution; off by default). language: 'bash' (default, `bash -lc`), 'python' (`ipython -c`), or 'node' (`node -e`). To run a SKILL's bundled scripts, pass `skill` = its id: the skill's files are staged into the working directory first, so the relative paths in its SKILL.md (e.g. `python scripts/office/unpack.py`) work as-written. Returns merged stdout/stderr, exit code, and duration; killed if it produces no output for the idle timeout or exceeds the max timeout.",
    parameters: [
