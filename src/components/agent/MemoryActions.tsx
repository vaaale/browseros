...
export function MemoryActions({ agentId = DEFAULT_AGENT_ID }: { agentId?: string }) {
  ...
  useCopilotAction({
    ...
    handler: ({ topic, content, digest }) =>
      runToolHandler("memory_save", async ({ signal }) => {
        ...
        if (digest && String(digest).trim()) {
          await fetchToolJson("memory_save", `/api/memory${q}`, {
            method: "POST",
        ...
        const out = await fetchToolJson("memory_save", `/api/memory${q}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  useCopilotAction({
    ...
    handler: ({ topic }) =>
      runToolHandler("memory_recall", async ({ signal }) => {
        if (topic && String(topic).trim()) {
          const slug = String(topic).trim();
          const out = await fetchToolJson("memory_recall", `/api/memory${q}&topic=${encodeURIComponent(slug)}`, { signal });
          if (!out.ok) {
            if (out.error.includes("HTTP 404")) {
        ...
        const out = await fetchToolJson("memory_recall", `/api/memory${q}`, { signal });
        if (!out.ok) return out.error;
        const res = out.data;
  ...
  useCopilotAction({
    ...
    handler: ({ query, maxResults }) =>
      runToolHandler("memory_search", async ({ signal }) => {
        ...
        const out = await fetchToolJson("memory_search", `/api/memory/search?${params.toString()}`, { signal });
        if (!out.ok) return out.error;
        const res = out.data;
        if (res.error) return `Error: ${res.error}`;
