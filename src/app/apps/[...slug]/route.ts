...
// Serves an installed item's app files through its dataDir()/system/app/<id>
// symlink (which points at the item's app/ folder under user-apps/ — see
// symlinkManager.ts). Apps load in an iframe at /apps/<id>/ and may call
// BrowserOS APIs (same-origin).
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await ctx.params;
  if (!slug || slug.length === 0) return new NextResponse("Not found", { status: 404 });
  const id = slug[0];
  if (!/^[a-zA-Z0-9._-]+$/.test(id) || id.startsWith(".")) return new NextResponse("Not found", { status: 404 });
