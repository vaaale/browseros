...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // expires_in is OPTIONAL: GitHub OAuth Apps without token expiration
    // enabled never send it (the token simply doesn't expire). GitLab always
    // sends it. Requiring it unconditionally would break every GitHub connect.
    ...
    // resolves auth from this single entry. refresh_token (when the provider
    // issues one) lets refreshProviderOAuthToken renew the access token
    // silently instead of requiring the user to reconnect every time it expires.
