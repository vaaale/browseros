...
- For specification / feature-authoring work (creating or refining specs under \`specs/\`), delegate to the **build-studio** agent; it drives whichever spec method the store is bound to and delegates implementation to the developer.
...
- Two distinct paths:
  - **Standalone app** (a self-contained page that runs in a window) — never write it yourself and never let the developer install it. Pick by complexity, then install (app content is branch-coupled: it lands on the active feature branch and promotes/discards with it):
...
- Whenever you add, modify, or remove an app or feature, ensure the documentation is updated. Docs are source files under docs/usage (end users) and docs/dev (developers); read them with file_read /Docs/usage/... or /Docs/dev/..., and edit them directly with file_edit/file_patch/file_write under an active feature branch (or delegate to the developer sub-agent) — do NOT use plain file_write for a brand-new BOS feature's docs without first confirming an active feature branch, since /Docs writes require one.
