...
- Self-modification: separate stable Supervisor owning the public port; one always-on **base** plus multiple branch-owned **previews** on pooled ports; per-session preview pin; safe-ordering promote (build + health-gate on the base port before moving the base ref); Assistant conversations carry an explicit Active feature branch for developer harness work; `/__supervisor` fallback page (Preview / Back to base / Promote / Stop / Discard / Push); code-only promote discarding the preview data clone. (Rollback is deferred — every promote leaves a `bos/v<timestamp>` tag as the anchor.)
