...
4. **Given** a promote that cannot proceed (base checkout dirty, not a fast-forward, or a merge conflict), **When** the user clicks Promote, **Then** it fails with an actionable message surfaced in the UI — never a silent no-op.
...
- **FR-007**: Promote MUST require a `verified` candidate (or override) AND its preconditions — the base checkout is clean (no uncommitted changes) and the candidate fast-forwards the base branch — failing early with an actionable, surfaced error otherwise; then fast-forward the feature branch into the base branch `active` tracks, re-anchor `active`, record `previous`, create a mandatory annotated tag (`bos/v<N>`), push per `pushMode` (`manual` default | `auto-on-promote`; manual push always available), and flip routing with drain; promote is **code-only** (data clone discarded, base carries forward).
