...
3. **Given** a candidate built by a delegated developer-agent fix (not via the branch dropdown), **When** the user opens the Topbar controls, **Then** the candidate is NOT auto-served — the toolbar shows it is a candidate and offers an explicit **Preview** (the user is still on `active` until they preview), so "the fix is in but the app is unchanged" cannot happen silently.
4. **Given** the base advanced under a `ready` candidate (promote is not a fast-forward) and the candidate rebases cleanly, **When** the user clicks Promote, **Then** the system rebases the candidate onto base, **rebuilds and re-health-gates** it, and a subsequent Promote is a clean fast-forward of built-and-tested code (the user is told it is refreshing — never a silent no-op).
...
- **FR-005**: The Supervisor MUST reverse-proxy the public port to `active` by default; a per-session preview pin (`POST /__supervisor/pin`, session cookie) routes a session to another version without moving `active`; preview is gated on health; a previewed candidate uses an isolated data clone. A candidate provisioned by a delegated developer-agent run MUST NOT be auto-pinned — the user previews it explicitly. The Supervisor MUST report which version a session is currently being served (`serving` in `/__supervisor/state`, resolved from the pin) so the UI can distinguish "previewing the candidate" from "a candidate exists but you're still on `active`". `GET /__supervisor/next-changes` MUST report the candidate's changed files (committed in its worktree) so an assistant's `gitStatus` is not fooled by a clean main checkout.
- **FR-006**: The Supervisor MUST serve a version-independent control page at `/__supervisor` (plain HTML) that remains reachable when a BOS UI is broken (show state, pin/clear, rollback, push to remote); the Topbar MUST surface convenient controls (Preview/Back/Promote/Push/Discard) but the system MUST NOT depend on it alone. Topbar control failures (promote/discard/preview/activate) MUST be surfaced to the user, never silently swallowed.
- **FR-007**: Promote MUST require a `verified` candidate (or override) and a **clean base checkout** (no uncommitted changes), failing early with an actionable, surfaced error otherwise. Promote then **escalates**: (a) if the candidate fast-forwards the base, fast-forward the feature branch into the base branch `active` tracks; (b) **if the base moved under the candidate** (not a fast-forward) and the candidate rebases cleanly, **refresh** it — rebase the candidate worktree onto base, then **rebuild + re-health-gate** so the eventual fast-forward serves built-and-tested code (not the stale build) — after which a fast-forward promote applies; (c) if the rebase would conflict, fail with the conflicting files surfaced for manual resolution (in-browser 3-way merge is a **deferred** capability). On a successful fast-forward, re-anchor `active`, record `previous`, create a mandatory annotated tag (`bos/v<N>`), push per `pushMode` (`manual` default | `auto-on-promote`; manual push always available), and flip routing with drain; promote is **code-only** (data clone discarded, base carries forward).
