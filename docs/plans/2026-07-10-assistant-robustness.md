...
### e2e suite state-dependence (2026-07-10, for Phase 4/6)

The full suite shares the dev server AND the persistent `data/` dir, so results are order/state-dependent: `per-agent-render-gate.spec.ts` fails when the server-side per-agent tool gate silently falls back to "no gate" (`/api/copilotkit` route catches `getAgent()` errors silently → the wildcard renderer then shows cards for disallowed tools). It passed and failed at the SAME commit depending on accumulated data-state. Build Studio's `data/agents/build-studio/AGENT.md` allowlist verified intact (readDoc in, launchApp out). Needs: (a) e2e data isolation (per-run BOS_DATA_DIR), (b) the copilotkit route should log (not swallow) agent-resolution failures. Also: `app-candidate` Discard test is flaky in full-suite runs for the same shared-state reasons. Live specs (`*-live.spec.ts`) require a live LLM and must be excluded in CI-style runs (`--grep-invert live`).

### card-collapse "regression" — resolved as stale test (2026-07-10)

The accordion was NEVER broken: instrumentation showed correct registration order (tool card closed, answer open). The failing assertion checked DOM ABSENCE (`toHaveCount(0)`) of the card body, but the card had been rewritten (pre-branch) from conditional render to an animated CSS collapse that keeps the body mounted. Fixes: body gets `invisible` when closed (ChatToolRenderer.tsx — genuinely imperceivable + excluded from visibility queries); test now asserts `aria-expanded` + `toBeHidden()`/`toBeVisible()`. Earlier mount-order/liveness theories in this plan's findings were wrong — reverted.
