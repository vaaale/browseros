...
- **FR-006**: BOS MUST support multiple agents as the assistant's personality. The personality is **per-conversation** — each conversation carries its own `agentId` (selected via the Assistant app's agent drop-down / conversation-group "+"), and different conversations open at once MAY run different agents independently; there is NO single global "active agent" (see `019`). The assistant can do anything in BOS (build apps/features, configure settings, open apps) and MUST always delegate substantive tasks to a sub-agent (creating one if none fits).
...
- **FR-008**: The Assistant app MUST have a right side panel (tabs: tools, skills, MCP servers, and session **State** — the CopilotKit AG-UI shared state, per `019`) and a left side panel for conversation management (new/delete/select/resume).
...
- **FR-011**: A Developer sub-agent MUST modify BOS's own source (`src/`) only through the Supervisor-backed Dev Harness on a validated active `bos/<kebab-name>` feature branch; standalone apps are produced by the Developer and installed via `installApp`/`buildApp`; the VFS is the user's sandbox and is NOT BOS source.
...
- Detailed sub-systems: `001-build-studio`, `002-memory`, `003-self-improvement`, `004-browser-automation`, `005-self-modification`, `006-data-isolation`, `007-gitfs`, `008-self-testing`, `009-installed-apps`, `010-documentation`, `011`–`019` (per-agent capabilities, embedded assistant, MCP gateway, unified agents, tool taxonomy + sandbox). Principles: the constitution. Drift: `discrepancies.md`.
