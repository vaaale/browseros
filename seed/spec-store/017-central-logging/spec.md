...
### User Story 8 - Conversation tracking and payload control (Priority: P2)

Users can track conversation-level activity with higher fidelity logging, including the ability to toggle payload logging for sensitive data.

**Acceptance Scenarios**:

1. **Given** a chat message or tool call/response, **When** it is logged, **Then** the `conversation` field is populated with a unique conversation identifier.
2. **Given** the "Log payload" setting is enabled, **When** a chat message is logged, **Then** the full payload is included in the log record.
3. **Given** the "Log payload" setting is disabled, **When** a chat message is logged, **Then** only metadata (without payload) is logged.
4. **Given** the log viewer in Settings, **When** filtering logs, **Then** the user can filter by `component` using free-text matching.
5. **Given** the log viewer in Settings, **When** filtering logs, **Then** the user can filter by `conversation` using free-text matching.
...
- **FR-004**: Each record MUST be one JSON object per line (JSONL) with at least: `ts`, `level` (`debug|info|warn|error`), `stream` (`frontend|backend|supervisor`), `sessionId?`, `versionLabel` (`base|preview|dev`), `component`, `conversation?`, `msg`, optional `data`, `err` (message + stack), and `branch?`. The `conversation` field MUST be populated for all messages stemming from a conversation (chat messages, tool calls/responses).
...
- **FR-011**: The Supervisor MUST serve reads at `GET /__supervisor/logs?session=&stream=&level=&component=&conversation=&since=`, returning a time-ordered stream. A **Logs** surface in BOS MUST default to the **complete timeline** and let the user narrow by session, stream, level, component, conversation, branch, or time using free-text matching for component and conversation filters. Reads MUST go through the sink (the Supervisor), consistent with it being the single owner of the store.
