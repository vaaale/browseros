...
- **FR-008**: `run_command` MUST execute all commands via `bash -lc` and support a `skill=<id>` argument that stages that skill's bundled files into `/workspace` before running (so `SKILL.md` relative script paths resolve). No `language` parameter — callers invoke `python3`, `node`, `pip3`, etc. directly in the shell command.
