...
- Q: How are the (dynamically-shaped) tool arguments passed to `callMcpServerTool`? → A: As a **JSON object string**, NOT an object parameter. CopilotKit converts a bare `object` action parameter (no declared sub-properties) into a closed JSON schema, so the model's keys get stripped to `{}` before the call is made (observed: every `update_issue` call arrived with empty `args` even though the model had the right values). A string parameter passes through untouched and is `JSON.parse`d server/handler-side.
...
- **FR-004**: The agent MUST have **`callMcpServerTool(server, tool, args)`** that executes the named tool server-side (reusing the existing MCP client + result extraction, so text and MCP-UI results pass through) and returns its result, with a clear error when the server/tool is unknown, disallowed, or unreachable. `args` MUST be accepted as a **JSON object string** (then parsed), because the chat framework strips the keys of an unstructured object parameter — passing arbitrary, per-tool-shaped arguments any other way silently yields `{}`.
