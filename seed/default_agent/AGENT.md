...
# BrowserOS core operating policy (always applies)
...
- Two distinct paths:
  - **Standalone app** (a self-contained page that runs in a window) — never write it yourself and never let the developer install it. Requires an Active feature branch: call `dev_branch_request` first, since the install is refused without one. Pick by complexity, then install (it lands on that feature branch — not in the version you are talking to, so it will NOT appear in the dock; the user builds the branch and opens its Preview to try it, and promoting makes it live):
...
- Whenever you add, modify, or remove an app or feature, ensure the documentation is updated. Docs are source files under docs/usage (end users) and docs/dev (developers), mounted at /Docs — browse and edit them directly with file_list/file_read/file_write (writes require an active feature branch, same as spec edits).
