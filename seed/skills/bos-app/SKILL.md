...
- **Marketplace item** (`data/user-apps/items/<id>/` — app facet, service facet, docs facet, or all of them): follow `references/target-marketplace-item.md` from the Build Studio skill instead. In short: call `dev_branch_request` FIRST (item content is branch-coupled now, and every `app_spec_*` write and install is refused without an active branch) but NOT `dev_delegate`. Then call `agent_delegate` (`agent:"developer"`, `contentOnly:true`) to get either a self-contained `index.html` or a staged multi-facet project (app under `app/`, a background service under `services/`, and the item's documentation under the staging dir's own `docs/usage/<Name>/` + `docs/dev/<Name>/` — all of these paths are relative to the staging directory, i.e. inside the item, NOT in BOS's source tree; exact task phrasing and trigger-phrase rules are in that reference, getting the wording wrong gets the delegation refused), then call `app_install` or `app_build` YOURSELF to finish the install — one call installs every facet the staged item has, and lands it on the active feature branch (NOT in the version you are talking to, so it will not appear in the dock; the user builds that branch and opens its Preview to try it). Summarize what was built (including any service facet — point the user at Settings → Plugins → Services for that, and the Docs app for the item's pages), call `buildstudio_tree_refresh()`, and use `app_list`/`bos_app_launch` to confirm and show the app facet if there is one.

  Documentation is part of the deliverable, not a followup: an item carries its own docs INSIDE the item — its own `docs/usage/<Name>/` and `docs/dev/<Name>/`, which are the item's paths and have nothing to do with BOS's `docs/usage`/`docs/dev` source trees; BOS's Docs app merges the two for display, which is the only reason they look like one tree. An item built without them is not finished — see "The `docs/` facet" in `target-marketplace-item.md` for the layout. If this item replaces something that used to live in BOS core, the superseded pages in BOS's OWN `docs/` must be deleted in the same job (a separate ordinary `dev_delegate` — the only part of this job that touches BOS's docs tree at all).
