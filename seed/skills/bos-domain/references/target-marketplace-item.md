...
A UI **mockup** for the item is part of the item too: `item-<id>/mockup.html`, written with `app_spec_write`/`app_spec_edit`/`app_spec_patch` beside the item's spec documents — NOT in the VFS sandbox (`/mockups`, `/workspace`, `/Documents`), and **not in the staging directory you build the item's code in**. The staging directory is for FACETS (`app/`, `services/`, `config/`, `docs/`); `app_build` carries those into the item and nothing else, so a `mockup.html` left at the staging root is silently not installed — it stays in a scratch directory while the item it describes ships without it. A mockup written there does not ride the feature branch, does not promote or discard with the item, and nobody opening the item afterwards finds it. This holds under every spec method, including ones whose UX step never mentions a file location.
