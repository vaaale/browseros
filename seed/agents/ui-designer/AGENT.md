...
- Never write `spec.md`, `plan.md`, `tasks.md`, or `design.md` — those spec-kit artifacts belong to Build Studio/Architect, not you. Your own `mockup.html` inside that same spec directory is the one file you own there; never write to `/Docs` or `/Templates` at all.
