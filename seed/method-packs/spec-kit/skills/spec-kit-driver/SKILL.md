...
description: Drive the SPEC-KIT pipeline to author and refine BOS specifications, then delegate implementation to the Developer. This is spec-kit's driver — a store bound to another method has its own.
when_to_use: When the store or project in play is bound to SPEC-KIT and you are running any of its steps (constitution, specify, clarify, design, plan, tasks, analyze, implement, converge). Load it because `methods_list` named it as the method's driverSkill, not by assumption — under BMAD or OpenSpec a different driver applies.
created_by: seed
pinned: true
---

The Build Studio skill drives the spec-kit pipeline. **Where a spec's artifacts live depends on its target — this is a physical split, not just a field.** For `bos-core`/`builtin-app`/`n/a`: specs live in external git-backed STORES under BOS_SPECS_ROOT — a BOS-owned system store (id 'bos-system-specs') and your writable 'user-specs' store — reachable through the ordinary `file_*` tools via VFS mounts: `/Specs/bos-system-specs`, `/Specs/user-specs`, `/Methods/spec-kit/templates` (read-only — this method's engine: command prompts and blank templates), and `/Docs` (docs/). Discover stores with `file_list('/Specs')`. Governing principles live in the system store at `/Specs/bos-system-specs/.specify/memory/constitution.md`; per-feature artifacts live at `/Specs/<store>/<NNN-feature>/` (spec.md, plan.md, tasks.md, ...). For `marketplace-item`: ALL artifacts (spec.md, design.md, plan.md, tasks.md, ...) live INSIDE the item itself, addressed as `item-<id>/<artifact>.md`, reachable ONLY through the `app_spec_create`/`app_spec_list`/`app_spec_read`/`app_spec_write`/`app_spec_edit`/`app_spec_patch` tools — never `file_*`, never `/Specs`. See `references/specify.md` and `references/target-marketplace-item.md`.
...
|---|---|---|---|---|---|
| BOS itself (settings, API routes, server logic, desktop) | `bos-core` | `bos-domain`: `references/target-bos-core.md` | `dev_delegate` | Yes — `dev_branch_request` first | `/Specs/bos-system-specs/<NNN-id>/` (`file_*`) |
| A built-in app under `src/apps/<id>/` | `builtin-app` | `bos-domain`: `references/target-builtin-app.md` | `dev_delegate` | Yes — `dev_branch_request` first | `/Specs/user-specs/<NNN-id>/` (`file_*`) |
| An item in the user's private marketplace `data/user-apps/items/<id>/` — an app facet, a background-service facet, or BOTH together | `marketplace-item` | `bos-domain`: `references/target-marketplace-item.md` | `agent_delegate` (`contentOnly:true`) + `app_install`/`app_build` (installs every facet the item has in one call) | **Yes** — `dev_branch_request` first, before ANY `app_spec_*` write or install | INSIDE the item — `item-<id>/` (`app_spec_*` tools, never `file_*`) |
...
- Read the command prompt and template with `file_read` at `/Methods/spec-kit/templates/commands/<step>.md` and `/Methods/spec-kit/templates/<artifact>-template.md` (always via `file_*` — the active method's template mount is read-only regardless of target). Then author the artifact itself: for `bos-core`/`builtin-app`/`n/a`, at `/Specs/<store>/<NNN-feature>/<artifact>.md` via `file_write`/`file_edit`/`file_patch`; for `marketplace-item`, at `item-<id>/<artifact>.md` via `app_spec_write`/`app_spec_edit`/`app_spec_patch` (`app_spec_create` only for the very first `spec.md`, which brings the item into existence). Either way: use the "write" tool ONLY to create a new artifact (or an intentional full rewrite); to MODIFY an existing artifact — adding a section, updating requirements, appending clarifications — read it first, then make targeted changes with the matching "edit" (one change) or "patch" (several ordered find/replace hunks in one atomic call) tool. Never rewrite a whole file just to add or tweak content. When you build the `find` text for `file_edit`/`file_patch`, copy it **verbatim from what `file_read` just returned** — do not retype or "improve" it from memory. The match is exact, so one stray or missing character (a dropped `---` section divider, a blank line, an em-dash vs hyphen, a trailing space, a markdown `**` marker) makes the hunk fail with `Hunk not found` and — because a patch is atomic — aborts the whole call with nothing written. Prefer a short, unique, single-line anchor per hunk over a long multi-line one; if an anchor must cross a section boundary, include every line in between exactly as you read it. The `find` for `file_edit`/`file_patch` is a byte-exact substring match — the tool refuses when it occurs 0 or more than 1 time — so copy the `find` string verbatim from the file contents returned by `file_read`/`app_spec_read` this session; never re-type it from memory, paraphrase it, or hand-shorten it (a `find` shown in the error message ending in `…` is over 60 chars and almost always means you typed a version that differs from the file's actual bytes). Prefer a short, unique, single-line anchor that does not span a section divider (`---`) or a blank line; a re-typed multi-line `find` that drops or alters an interior line is the most common cause of a spurious `Text not found` / `Hunk not found` refusal (this recurred on the 031-self-healing spec — EHS-0003 via `file_patch`, EHS-0004 via `file_edit`). Your `file_*` tools only reach VFS mounts (`/Specs`, `/Docs`, `/Methods/<id>/templates`, and the user's sandboxed files) — never BOS source, and never `data/user-apps` (that's what `app_spec_*` is for).
