...
3. **Create the spec feature directory**:
   ...
   **Resolution order for `SPECIFY_FEATURE_DIRECTORY`**:
   ...
   2. Otherwise, auto-generate it under `specs/`:
      - Numbering comes from the active method descriptor's `sections[].numbering` (046): `"nnn-slug"` allocates the next available 3-digit prefix within the Project, `"none"` uses the slug alone. BOS allocates it — do not scan directories yourself.
      - Construct the directory name accordingly (e.g. `003-user-auth`)
      - Set `SPECIFY_FEATURE_DIRECTORY` to the Project-relative path
   ...
   **Create the directory and spec file**:
   ...
   - Downstream commands (`__SPECKIT_COMMAND_PLAN__`, `__SPECKIT_COMMAND_TASKS__`, etc.) locate the feature from the store-prefixed path BOS already tracks — you do not need to persist it anywhere.
